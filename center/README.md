...
```bash
sudo deploy/install.sh install    # спросит порт, пользователя сервиса, сетевые адреса (список
                                   # реальных интерфейсов хоста + опция поднять VPN-хаб)
```
...
В веб-интерфейсе: «Управление агентами» → «Добавить агента» — укажите только имя. Джейлы
вводить не нужно — агент сам находит их на хосте (`fail2ban-client status`) при установке
и держит список актуальным на каждом чекине. Токен показывается один раз сразу после
создания. Он же — через CLI:
